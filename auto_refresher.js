...
// Refresher
function Preload(){if(Number(GM_getValue(window.location.href))){if(GM_getValue("debug")){console.log("will refresh in "+formatTime(wait)+" if refresh still on")}setTimeout(()=>{if(GM_getValue(window.location.href)){if(GM_getValue("debug")){console.log("refreshed in "+formatTime(new Date().valueOf()-lastRenew))};lastRenew=new Date().valueOf();window.location.reload()}},betterRandom(GM_getValue("min")||0,GM_getValue("max")||1))}}

// Main Loop
function load(){var startData=GM_getValue(window.location.href);const loop=setInterval(()=>{if(!(GM_getValue(window.location.href)==startData)){clearInterval(loop);if(Number(GM_getValue(window.location.href))){if(GM_getValue("debug")){console.log("refreshed loop in "+formatTime(new Date().valueOf()-lastRenew))};lastRenew=new Date().valueOf();load()}}else{if(GM_getValue("debug")){console.log("fired refresh in "+formatTime(new Date().valueOf()-lastRenew))};Preload()}},startData)}

// Precheck
if (GM_getValue(window.location.href)){load();if(GM_getValue("debug")){console.log("Refresher Is Online "+formatTime(GM_getValue(window.location.href)))}}

// Toggle
GM_registerMenuCommand("Toggle Site Refresher",()=>{if(!Number(GM_getValue(window.location.href))){var delay=prompt("Set Auto Refresh Rate","1000 (1 second in milisecond)");if(Number(delay)){GM_setValue(window.location.href, delay);if(GM_getValue("debug")){console.log("started loop in "+formatTime(new Date().valueOf()-lastRenew))};lastRenew=new Date().valueOf();load()}}else{GM_deleteValue(window.location.href)}alert("Auto Site Refresher Is "+(GM_getValue(window.location.href)&&"Online And Set To "+GM_getValue(window.location.href)+"ms"||"Offline"))})
